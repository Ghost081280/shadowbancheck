// =============================================================================
// HASHTAG CHECKER - PAGE-SPECIFIC FUNCTIONALITY
// =============================================================================
...
const BANNED_HASHTAGS = {
    banned: {
        instagram: ['adult', 'adulting', 'alone', 'armparty', 'asiangirl', 'ass', 'assday', 'babe', 'bbc', 'beautyblogger', 'besties', 'bikinibody', 'boho', 'boobs', 'brain', 'costumes', 'curvy', 'date', 'dating', 'desk', 'direct', 'dm', 'dogsofinstagram', 'eggplant', 'elevator', 'fishnets', 'fitfam', 'girlsonly', 'gloves', 'goddess', 'happythanksgiving', 'hardworkpaysoff', 'hawks', 'hotweather', 'hustler', 'ig', 'instababy', 'instamood', 'iphone', 'italiano', 'kansas', 'kickoff', 'killingit', 'kissing', 'lean', 'like', 'likeforlike', 'likes', 'loseweight', 'master', 'mileycyrus', 'models', 'mustfollow', 'nasty', 'newyearsday', 'nudity', 'overnight', 'petite', 'pornfood', 'prettygirl', 'pushups', 'rate', 'saltwater', 'selfharm', 'sexy', 'single', 'singlelife', 'skateboarding', 'skype', 'snap', 'snapchat', 'snowstorm', 'sopretty', 'stranger', 'streetphoto', 'stud', 'sunbathing', 'swole', 'tag4like', 'tagsforlikes', 'teen', 'teens', 'thought', 'todayimwearing', 'twerking', 'underage', 'valentinesday', 'weed', 'workflow', 'wtf'],
...
const TRENDING_BANNED = [
    ...
    { tag: 'goddess', platforms: ['instagram'], reason: 'Sexualized content' },
    { tag: 'instababe', platforms: ['instagram'], reason: 'Sexualized content' },
    { tag: 'hotgirl', platforms: ['instagram'], reason: 'Sexualized content' },
    { tag: 'lingerie', platforms: ['instagram'], reason: 'Adult content' },
    { tag: 'thighs', platforms: ['instagram'], reason: 'Sexualized content' },
    { tag: 'thinspiration', platforms: ['instagram'], reason: 'Pro-ED content' },
    { tag: 'bossbabe', platforms: ['facebook'], reason: 'MLM spam' },
    { tag: 'hunlife', platforms: ['facebook'], reason: 'MLM spam' },
    { tag: 'sidehustle', platforms: ['facebook'], reason: 'MLM spam' },
    { tag: 'workfromhome', platforms: ['facebook'], reason: 'Scam risk' }
];
...
function checkHashtag(tag) {
    ...
    const result = {
        tag: tag,
        status: 'safe',
        platforms: { instagram: 'safe', tiktok: 'safe', twitter: 'safe', facebook: 'safe' },
        reason: null
    };
    ...
    content.innerHTML = `
            ...
            ${results.map(r => `
                <div class="result-tag ${r.status}">
                    ...
                    <span class="platforms">
                        <span title="Instagram">${r.platforms.instagram !== 'safe' ? '📸' : ''}</span>
                        <span title="TikTok">${r.platforms.tiktok !== 'safe' ? '🎵' : ''}</span>
                        <span title="Twitter">${r.platforms.twitter !== 'safe' ? '🐦' : ''}</span>
                        <span title="Facebook">${r.platforms.facebook !== 'safe' ? '📘' : ''}</span>
                    </span>
                </div>
            `).join('')}
        </div>
        ...
        ${banned.length > 0 ? `
            <div class="result-cta" style="margin-top: var(--space-lg);">
                ...
                <p>Remove these before posting to avoid shadow bans. Get Shadow AI Pro for alternative suggestions.</p>
                <a href="index.html#shadow-ai-pro" class="btn btn-primary" style="margin-top: var(--space-md);">Get AI Suggestions →</a>
            </div>
        ` : ''}
    `;
...
window.exportResults = function() {
    if (!window.lastResults) return;
    const lines = ['HASHTAG ANALYSIS - ShadowBanCheck.io', '', ''];
    ['safe', 'banned', 'restricted'].forEach(status => {
        ...
        if (filtered.length > 0) {
            ...
            filtered.forEach(r => lines.push(`#${r.tag}`));
            lines.push('');
        }
    });
        ...
        grid.innerHTML = toShow.map(t => `
            <span class="trending-tag" title="${t.reason}">
                ...
                <span class="tag-platforms">
                    ${t.platforms.includes('instagram') ? '📸' : ''}${t.platforms.includes('tiktok') ? '🎵' : ''}${t.platforms.includes('twitter') ? '🐦' : ''}${t.platforms.includes('facebook') ? '📘' : ''}
                </span>
            </span>
        `).join('');
...
function updateTrendingDate() {
    const el = document.getElementById('trending-date');
    if (el) {
...
function updateCounterDisplay() {
    ...
    el.innerHTML = remaining === 0 
        ? `<span style="color: #ef4444;">0 / ${MAX_HASHTAG_SEARCHES}</span>` 
        : `${remaining} / ${MAX_HASHTAG_SEARCHES}`;
}
